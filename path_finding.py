...
for i in range(0,row):
    grid[0][i].show(blue, 0)
    grid[0][i].obs = True
    grid[cols-1][i].obs = True
    grid[cols-1][i].show(blue, 0)
    grid[i][row-1].show(blue, 0)
    grid[i][0].show(blue,0)
    grid[i][0].obs = True
...
def main():
    ...
    if len(open_set) > 0:
        ...
        if current == end:
            ...
            Tk().wm_withdraw()
            result = messagebox.askokcancel('Program Finished', ('The program finsihed, the shortest distance \n to the path is ' + str(temp) + ' blocks away, \n would you like to re run the program?'))
            if result == True:
